 ...
 */
public abstract class LoginEmailRequest extends AsyncTask<String, Void, String> {

    public String aux = "";

    protected String doInBackground(String... urls) {
        try {
            URL url = new URL(Utils.URL + "/users/login/email"); //the URL we will send the request to
        ...
        if (!aux.equals("")){
            try {
                ...
                if(error == 0){
                    ...
                    onFailed("Error: Email y/o contraseña incorrecto.");
                }
                //read(intAux);
            } catch (JSONException e) {
                e.printStackTrace();
