 ...
 */
public abstract class NewIncidenceRequest extends AsyncTask<String, Void, String> {
    public String aux = "";

    protected String doInBackground(String... urls) {
        try {
            URL url = new URL(Utils.URL + "/incidencias/save"); //the URL we will send the request to
    ...
    protected void onPostExecute(String urls) {
        ...
        if (!aux.equals("")) {
            try {
                ...
                if (error == 0) {
                    String incidenceId = json.optString("user");
                    onSuccess(incidenceId);
                } else if (error == 1) {
                    onFailed("Error: El correo electrónico introducido ya está en uso.");
                } else if (error == 2) {
                    onFailed("Ha ocurrido un error en el servidor, vuelva a intentarlo más tarde.");
                }
                //read(intAux);
            } catch (JSONException e) {
                e.printStackTrace();
    ...
    public abstract void onSuccess(String incidenceId);
